...
const UNDEFINED = undefined,
   ...
   */
  isDisabledDate = (disabledDates = [], date) => {
    let i,
  ...
  displayMonth = calendar => {
    const { info, graphic, classNames } = calendar,
      { active, rangeStart, rangeEnd, weekStartingDay, highlight, highlightClasses, disabledDates } = info,
    ...
    for (i = 0, l = dateElements.length; i < l; i++) {
      if (i < monthStaringWeekDay) {
        ...
        if (isDisabledDate(disabledDates, { day: j, month, year })) {
...
class Calendar {
  ...
  configure (config, doRepaint) {
    ...
    if (config.disabledDates) {
      info.disabledDates = config.disabledDates.map(dateObj => Object.assign(dateObj));
      doRepaint = true;
    }
