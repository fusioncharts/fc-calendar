...
/**
 * A object representing the date that the FusionCalender understands
 * @typedef {object} dateObj
 * @property {number} dateObj.day representing the date of the date object.
 * @property {number} dateObj.month representing the month of the date object.
 * @property {number} dateObj.year representing the year of the date object.
 */
...
class Calendar {
  /**
   * Create a calendar instance to display a calendar in your page.
   * @param {object} [config] - The configuration object for the calendar.
   */
  constructor (config) {
    const calendar = this,
      today = new Date(),
  ...
  /**
   * Configure the Calendar instance.
   * @param {object} config - The configurations for the calendar.
   * @param {boolean} [forceReDraw = flase] - Whether the calender will be forcefully re-drawn even there is no config related to the visuals
   * @param {number} [config.width = 300] - The width of the calendar.
   * @param {number} [config.posX = 0] - X position shift.
   * @param {number} [config.posY = 0] - Y position shift.
   * @param {string} [config.vAlignment = top] - Vertical alighment of the calendar box w.r.t it's posX and posY.
  ...
  configure (config, forceReDraw) {
    ...
    forceReDraw && displayMonth(calendar);
   ...
   * @return {dateObj} the Dateobject of teh selected date.
   */
  getDate () {
    return this.info.selectedDate;
  }
  /**
   * Set listener for various calendar events.
   * @param {string} eventName - The name of the event.
   * @param {function} listener - The listener function for the event
   */
  addEventListner (eventName, listener) {
    typeof listener === 'function' && (this.events && (this.events[eventName] = listener));
  }
  /**
   * Remove listener for a calendar events.
   * @param {string} eventName - The name of the event for which the listner will be removed.
   */
